...
int dissect_t(const element_meta* val_meta, dissector d, context* ctx) {
    return dissect_elem_mandatory(val_meta, std::move(d), dissect_opt_t, ctx);
}

int dissect_lv(const element_meta* val_meta, dissector d, context* ctx) {
    return dissect_elem_mandatory(val_meta, std::move(d), dissect_opt_lv, ctx);
}
int dissect_lv_e(const element_meta* val_meta, dissector d, context* ctx) {
    return dissect_elem_mandatory(val_meta, std::move(d), dissect_opt_lv_e, ctx);
}
int dissect_v(const element_meta* val_meta, dissector d, context* ctx) {
    return dissect_elem_mandatory(val_meta, std::move(d), dissect_opt_v, ctx);
}
int dissect_tv_short(const element_meta* val_meta, dissector d, context* ctx) {
    return dissect_elem_mandatory(val_meta, std::move(d), dissect_opt_tv_short, ctx);
}
int dissect_tv(const element_meta* val_meta, dissector d, context* ctx) {
    unused(dissect_tv);
    return dissect_elem_mandatory(val_meta, std::move(d), dissect_opt_tv, ctx);
}

int dissect_tlv(const element_meta* val_meta, dissector d, context* ctx) {
    unused(dissect_tlv);
    return dissect_elem_mandatory(val_meta, std::move(d), dissect_opt_tlv, ctx);
}

/*  Type (T) element dissector */
int dissect_opt_t(const element_meta* val_meta, dissector d, context* ctx) {
...
int dissect_opt_lv(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
...
int dissect_opt_lv_e(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
 ...
 */
int dissect_opt_v(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
 ...
 * is not defined for these.
 */
int dissect_opt_tv_short(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
 ...
 * is not defined for these.
 */
int dissect_opt_tv(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
...
int dissect_opt_tlv(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
 ...
 * octet 2a length
 */
int dissect_opt_telv(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
 ...
 */
int dissect_opt_tlv_e(const element_meta* val_meta, dissector d, context* ctx) {
    if (d.length <= 0) return not_present_diag(0, val_meta, ctx);
...
int dissect_tlv_e(const element_meta* val_meta, dissector d, context* ctx) {
