...
result_t de_authentication_request(dissector                 d,
    ...
    de_opt_t_octet(d.slice(17), ctx, 0x21, &ret->rand).step(d);
    de_opt_tl_octet(d.slice(18), ctx, 0x20, &ret->autn).step(d);
...
result_t de_authentication_response(dissector                  d,
    ...
    de_nmm_header(d, ctx, &ret->header).step(d);
    de_tlv_octet(d, ctx, 0x2d, ret->parameter.v, &ret->parameter.present).step(d);
    de_opt_tlv(d, ctx, 0x78, &ret->eap).step(d);
...
result_t de_authentication_result(dissector                d,
    ...
    de_nmm_header(d, ctx, &ret->header).step(d);
    de_nibble(d, ctx, &ret->nksi).step(d);
    de_lv_e(d, ctx, &ret->eap).step(d);
    de_tlv(d, ctx, &ret->abba).step(d);
...
result_t de_authentication_failure(dissector               d,
    ...
    de_tlv_octet(d, ctx, 0x30, ret->parameter.v, &ret->parameter.present).step(d);
...
result_t de_registration_request(dissector d, context* ctx, registration_request_t* ret) {
    ...
    de_nibble(d, ctx, &ret->nksi).step(d);
    de_lv_e< nmid_t >(d, ctx, &ret->nmid, dissect_nmid).step(d);
    ...
    de_t_octet(
        d.slice(7), ctx, 0x52, &ret->last_visited_tai.v, &ret->last_visited_tai.present)
        .step(d);
    return {d.offset - uc.offset};
}
...
bd	T3447 value	GPRS timer 3	9.11.2.5	O	TLV	3
...
struct service_accept_t {
    nmm_header_t                               header;
    opt_t< pdu_session_status_t >              pdu_session_status;              // 50 TLV 4+
    opt_t< pdu_session_reactivation_result_t > pdu_session_reactivation_result; // 26 TLV 4+
    opt_t< pdu_session_reactivation_result_error_cause_t > result_error_cause;  // 72 TLV-E 5+
...
struct configuration_update_command_t {
    ...
    opt_t< ntracking_area_id_list_t > tais;                                         // 54 TLV 9+
    ...
    opt_t< octet_t >                  operator_defined_access_category_definitions; // 76 TLVE 3+
...
struct notification_response_t {
    nmm_header_t                  header;
    opt_t< pdu_session_status_t > pdu_session_status; // 50
};
...
struct security_mode_command_t {
    ...
    bit4_t                              nksi;                   // 9.11.3.32 1/2
    ue_security_capability_t            replayed_ue_security_capabilities;    // LV 3
