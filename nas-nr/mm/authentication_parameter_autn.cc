...
int mm::dissect_authentication_parameter_autn(dissector d, context* ctx) {
    ...
    const auto subtree = d.add_item(d.length, &hf_dtap_autn);
    use_tree   ut(d, subtree);

    (void) d.add_item(6, &hf_dtap_autn_sqn);
    d.step(6);

    (void) d.add_item(2, &hf_dtap_autn_amf);
    d.step(2);

    (void) d.add_item(8, &hf_dtap_autn_mac);
    d.step(8);
