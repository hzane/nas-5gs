...
const bool_field hf_upu_data_type = {
    "UPU data type",
    0x01,
...
const bool_field hf_ue_parameters_update_ack = {
    "ACK",
    0,
...
const bool_field hf_reregistration                = {
    "Re-registration (REG)",
    0,
...
int mm::dissect_ue_parameters_update_transparent_container(dissector d, context*ctx) {
    ...
    (void) d.add_item( &hf_upu_data_type);
    const auto uput = d.uint8() & 0x01u;
    if (uput) {
        (void) d.add_item(&hf_ue_parameters_update_ack);
        (void) d.add_item(&hf_reregistration);
    }
    d.step(1);
