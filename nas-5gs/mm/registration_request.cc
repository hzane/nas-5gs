...
int mm::dissect_registration_req(dissector d, context* ctx) {
    ...
    consumed = dissect_opt_tlv(nullptr, &ue_sec_cap, d, ctx);
    d.step(consumed);
    ...
    consumed = dissect_opt_tv(nullptr, &last_v_tai, d, ctx);
    d.step(consumed);

    /*17    S1 UE network capability  9.11.3.48    O    TLV  4-15 */
    // ELEM_OPT_TLV(0x17, , DE_EMM_UE_NET_CAP, NULL);
    consumed = dissect_opt_tlv(nullptr, &s1_ue_net_cap, d, ctx);
    d.step(consumed);
    ...
    consumed = dissect_opt_tlv_e(nullptr, &ladn_ind, d, ctx);
    d.step(consumed);
