...
int mm::sec_mode_cmd(dissector d, context* ctx) {
    ...
    //             DE_NAS_5GS_MM_NAS_SEC_ALGO,
    //             NULL,
    //             ei_nas_5gs_missing_mandatory_elemen);
    auto consumed = dissect_elem_v(nullptr, &sec_algo, d);
    d.offset += consumed;
    d.length -= consumed;
    ...
    //             " - ngKSI",
    //             ei_nas_5gs_missing_mandatory_elemen);
    consumed = dissect_elem_v(nullptr, &nksi, d);
    d.offset += consumed;
    d.length -= consumed;
    ...
    //              DE_NAS_5GS_MM_UE_SEC_CAP,
    //              " - Replayed UE security capabilities",
    //              ei_nas_5gs_missing_mandatory_elemen);
    consumed = dissect_elem_v(nullptr, &replayed_ue_sec_cap, d);
    d.offset += consumed;
    d.length -= consumed;

    /*E-    IMEISV request    IMEISV request     9.11.3.28    O    TV    1*/
    // ELEM_OPT_TV_SHORT(0xE0, NAS_PDU_TYPE_EMM, DE_EMM_IMEISV_REQ, NULL);
    consumed = dissect_elem_tv_short(nullptr, &imeisv_req, d);
    d.offset += consumed;
    d.length -= consumed;
    ...
    consumed = dissect_opt_elem_tv(nullptr, &selected_eps_sec_algo, d);
    d.offset += consumed;
    d.length -= consumed;
    ...
    consumed = dissect_opt_elem_tlv(nullptr, &a_sec_info, d);
    d.offset += consumed;
    d.length -= consumed;
    ...
    consumed = dissect_opt_elem_tlv(nullptr, &abba, d);
    d.offset += consumed;
    d.length -= consumed;
    ...
    consumed = dissect_opt_elem_tlv(nullptr, &reported_s1_ue_sec_cap, d);
    d.offset += consumed;
    d.length -= consumed;
